...
// ---------------------- CANCEL BOOKING ----------------------
app.patch('/api/v1/bookings/:id/cancel', auth, async (req, res) => {
  const bookingId = req.params.id;
  const userId = req.user.id;
  ...
  try {
    ...
    const findQ = Booking.findOne({ _id: bookingId, user_id: userId });
    ...
    if (booking.status === 'completed') {
      if (session) await session.abortTransaction();
      return res.status(400).send({ error: 'Cannot cancel completed booking' });
    }

    // Check if cancellation is allowed (e.g., not too close to start date)
    const now = new Date();
    const hoursDiff = (booking.start_date - now) / (1000 * 60 * 60);
    ...
    const days = getDateRange(booking.start_date, booking.end_date);
    ...
    for (const date of days) {
      let q = Availability.updateOne(
        ...
        { $inc: { available_units: booking.quantity } },
      );
      if (session) q = q.session(session);
      await q;
    }
...
// ---------------------- GET ROOM DETAILS ----------------------
app.get('/api/v1/rooms/:id', auth, async (req, res) => {
  try {
    ...
    res.send({ room: room.toPublicJSON() });
  } catch (err) {
    if (err.name === 'CastError') {
      return res.status(400).send({ error: 'Invalid room ID format' });
    }
...
app.get('/api/v1/rooms/:id/availability', auth, async (req, res) => {
  try {
    const roomId = req.params.id;
    const { start, end } = req.query;
    ...
    const availability = await Availability.findByRoomAndDateRange(roomId, startDate, endDate);

    const availabilityMap = availability.map(avail => ({
      date: avail.date,
